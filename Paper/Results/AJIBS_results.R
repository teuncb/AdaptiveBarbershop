...
mtext(expression("n of audible " * phantom("retunings") * "/" * phantom("deviations")), side = 4, line = 3, col=col_retdev)
...
par(ps = title_size + 7, font=2)
title(main= substitute(paste(bold("Proposed algorithm's effects on "), bolditalic("Ring-a-Ding-Ding"))), line=1.3)
#title(main="Proposed algorithm's effects on Ring-a Ding Ding")
#title(main=expression("Proposed algorithm's effects on " * phantom("Ring-a-Ding Ding")))
# par(font=2)
#title(main=expression(phantom("Proposed algorithm's effects on ") * "Ring-a-Ding Ding"))
par(ps = font_size)
